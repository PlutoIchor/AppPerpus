...
class PengembalianExport implements FromQuery,  WithMapping, WithHeadings, WithCustomStartCell,  WithEvents

{
    ...
    public function __construct($tanggal_awal,$tanggal_akhir, $identitas)
    {
    ...
    public function map($pengembalians): array
    {
        ...
        return [
            [
                ...
                $this->index++,
                $pengembalians->user->username,
                $pengembalians->buku->judul_buku,
                $pengembalians->tanggal_peminjaman,
    ...
    public function query()
    {
        ...
        return Peminjaman::query()->where('tanggal_peminjaman', '>=', $this->tanggal_awal)->where('tanggal_pengembalian', '<=', $this->tanggal_akhir);
    }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                $event->sheet->setCellValue('B1', 'Laporan Pengembalian')->getStyle('B1:G1')
                    ->getAlignment()
                ...
                $event->sheet->setCellValue('B2', "$this->tanggal_awal s/d $this->tanggal_akhir")->getStyle('B2:G2')
                    ->getAlignment()
                    ->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER);;
